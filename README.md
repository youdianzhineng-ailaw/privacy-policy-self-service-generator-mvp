...
<p>
  ...
  <img alt="Version" src="https://img.shields.io/badge/Version-v1.1.1-2e7d32">
...
`v1.1.1` 增加了复查报告中的法律法规按需检索能力。这个能力用于在复查报告里确认法律法规、监管规则、国家标准、条文编号、效力层级、发布机关或适用范围，不会让 MVP 协议正文变成逐条法条罗列。

如需启用该能力，请配置元典 MCP。配置地址：[https://open.chineselaw.com/mcp-config](https://open.chineselaw.com/mcp-config)。
...
- 只有复查报告需要确认具体依据时，才调用元典 MCP。
- 普通 MVP 正文起草不强制检索每一条法律法规。
- 元典 MCP 未配置、不可用、检索失败或结果无法判断时，复查报告应写明原因，并标记为“需进一步核验”。
- 不得编造条文编号、发布机关、实施状态或具体法律结论。
- 检索结果仅用于辅助复查，不替代律师、数据合规顾问或企业内部法务的正式审查。
...
7. 复查报告需要确认法律法规、监管规则、国家标准或具体条文时，按需使用元典 MCP 检索；无法检索或无法确认时，明确标记为“需进一步核验”。
...
| 版本 | 状态 | 说明 |
| --- | --- | --- |
| `v1.1.1` | 已发布 | 生成 MVP 最小合规版隐私协议，输出基础清单、后续完善事项和复查报告；复查报告需要确认依据时，按需使用元典 MCP 检索法律法规 |
